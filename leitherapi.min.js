...
return D}function g(a){var b=c.LeitherDb.transaction("res","readwrite").objectStore("res"),D=new hprose.Future;request=b.put(a);request.onerror=function(a){debug.log("setdbdata err",a);D.reject(a)};request.onsuccess=function(a){D.resolve(a.target.result);debug.log("setdbdata: ",a.target.result)};return D}function y(c,b,D){var f=new hprose.Future,g=function(a,c){var b="last";if("last"==c||"release"==c)b=a[c];return a.ResFile[b]}(JSON.parse(D),b);(function(b){for(var D=[];b<g.length;b++){var v=g[b];
""==v?debug.error("invliad key:",v):D.push(a(c,v))}hprose.Future.all(D).then(function(a){debug.log("all promise",a);f.resolve()},function(a){debug.log(a);f.reject(a)})})(0);return f}function u(a){c.loginData=a;c.sid=a.sid;c.user=a.user;c.username=c.user.name;c.bid=null!==a.user?a.user.id:c.uid;c.leClient=c.api;c.swarm=a.swarm}function R(a){0<a.length?c.currentIP=a:a=c.currentIP;c.api=r(a);l().then(function(a){debug.log("hprose ready",a);c.api.ready(function(a){debug.log("hprose ready ok");debug.log("G.uid=",
c.uid);f(a,c.AppName,c.AppBid,c.AppVer).then(function(){var v=x("login");c.postLogin=u;"undefined"!==typeof ENABLE_UAC&&ENABLE_UAC?b("UAC enabled, must redirect to the login page first"):a.login(c.uid,c.userppt).then(function(v){debug.log("login ok");u(v);c.swarm=v.swarm;debug.log("LeitherIsOK:",C());debug.log("login ok sid=",v.sid);debug.log("user= ",v.user);debug.log("swarm=",v.swarm);debug.log("appName=",c.AppName);b("after loaded app and logedin");c.Local||(debug.log("use remote files"),a.getresbyname(c.sid,
c.SystemBid,"LeitherApi",c.AppVer,{handler:icache.handler,ttl:0}).then(function(a){var b=new FileReader;b.onload=function(a){debug.log("leitherApi re get ok");localStorage.leitherApi=a.target.result};b.readAsText(new Blob([a]))}),a.hget(c.sid,c.AppBid,"applist",c.AppName,{handler:icache.handler,ttl:0}).then(function(a){debug.log("manifest re hget ok");g({id:c.AppName,data:a,tbname:c.ApptbName}).then()}))},v)})},x("api.ready"))},x("InitDb"))}function a(a,b){var f=new hprose.Future,h=document.createElement("script");
...
window.api=r;var X=["log","info","warn","error"];window.debug={};window.setLog=H;var c={ayFE:{},ayErr:{},IPList:[],InitFunc:[]};window.G=c;window.LeitherIsOK=C;window.RunApp=function(a,b){if(0!=arguments.length){"undefined"===typeof a.Log?H(!1):H(a.Log);c.I=a;c.appBid=a.AppBid;c.Running=!1;c.AppVer=a.AppVer?a.AppVer:"last";debug.log("RunApp");var f;if(f=0==b)debug.log("InitCfg"),c.Local=a.Local,c.SystemBid=a.SystemBid,c.AppBid=a.AppBid,c.IPList=a.IPList&&a.IPList.length&&0<a.IPList.length?a.IPList:
B(c.AppBid+"/iplist"),c.userppt=a.userppt||B(window.location.pathname+"/"+c.AppBid+"/ppt"),c.AppName=a.AppName||B(c.AppBid+"/appname"),c.uid=a.userid||B(window.location.pathname+"/"+c.AppBid+"/uid"),f=!1;f||(c.IPList.length<=b?console.error("iplist.length [",c.IPList.length,"]<ipnum[",b,"]"):R(c.IPList[b]))}};window.RunApp()},function(Q,E,J){var L;(function(H){(function(x,C,b){function B(a,b){return"object"!=typeof b&&(b=b()),Object.keys(b).forEach(function(d){a[d]=b[d]}),a}function l(a){return{from:function(b){return a.prototype=
Object.create(b.prototype),a.prototype.constructor=a,{extend:function(d){B(a.prototype,"object"!=typeof d?d(b.prototype):d)}}}}}function z(a,b){return b(a)}function g(a,ja){function ka(a){this._cfg={version:a,storesSource:null,dbschema:{},tables:{},contentUpgrade:null};this.stores({})}function r(a,c,e,m){var k,w,p,Z,la;if(0===a)Object.keys(T).forEach(function(a){A(c,a,T[a].primKey,T[a].indexes)}),k=G._createTransaction(V,ba,T),k.idbtrans=c,k.idbtrans.onerror=q(e,["populating database"]),k.on("error").subscribe(e),
d.newPSD(function(){d.PSD.trans=k;try{G.on("populate").fire(k)}catch(a){m.onerror=c.onerror=function(a){a.preventDefault()};try{c.abort()}catch(p){}c.db.close();e(a)}});else{if(w=[],p=ca.filter(function(p){return p._cfg.version===a})[0],!p)throw new I("Dexie specification of currently installed DB version is missing");T=G._dbSchema=p._cfg.dbschema;Z=!1;p=ca.filter(function(p){return p._cfg.version>a});p.forEach(function(a){var p=T,k=a._cfg.dbschema;xa(p,c);xa(k,c);T=G._dbSchema=k;p=M(p,k);p.add.forEach(function(a){w.push(function(p,
k){A(p,a[0],a[1].primKey,a[1].indexes);k()})});p.change.forEach(function(a){if(a.recreate)throw new I("Not yet support for changing primary key");w.push(function(p,k){var w=p.objectStore(a.name);a.add.forEach(function(a){x(w,a)});a.change.forEach(function(a){w.deleteIndex(a.name);x(w,a)});a.del.forEach(function(a){w.deleteIndex(a)});k()})});a._cfg.contentUpgrade&&w.push(function(p,w){var b,d;Z=!0;b=G._createTransaction(V,[].slice.call(p.db.objectStoreNames,0),k);b.idbtrans=p;d=0;b._promise=z(b._promise,
function(a){return function(p,k,b){function c(a){return function(){a.apply(this,arguments);0==--d&&w()}}return++d,a.call(this,p,function(a,p){arguments[0]=c(a);arguments[1]=c(p);k.apply(this,arguments)},b)}});p.onerror=q(e,["running upgrader function for version",a._cfg.version]);b.on("error").subscribe(e);a._cfg.contentUpgrade(b);0===d&&w()});Z&&(0<=navigator.userAgent.indexOf("Trident")||0<=navigator.userAgent.indexOf("MSIE"))||w.push(function(a,p){for(var w,d=0;d<a.db.objectStoreNames.length;++d)w=
a.db.objectStoreNames[d],null!==k[w]&&k[w]!==b||a.db.deleteObjectStore(w);p()})});la=function(){try{w.length?w.shift()(c,la):wa(T,c)}catch(a){m.onerror=c.onerror=function(a){a.preventDefault()};try{c.abort()}catch(p){}c.db.close();e(a)}};la()}}function M(a,b){var d={del:[],add:[],change:[]},c,k,w,p,Z,e,q,m;for(c in a)b[c]||d.del.push(c);for(c in b)if(k=a[c],w=b[c],k)if(p={name:c,def:b[c],recreate:!1,del:[],add:[],change:[]},k.primKey.src!==w.primKey.src)p.recreate=!0,d.change.push(p);else{Z=k.indexes.reduce(function(a,
p){return a[p.name]=p,a},{});w=w.indexes.reduce(function(a,p){return a[p.name]=p,a},{});for(e in Z)w[e]||p.del.push(e);for(e in w)q=Z[e],m=w[e],q?q.src!==m.src&&p.change.push(m):p.add.push(m);(p.recreate||0<p.del.length||0<p.add.length||0<p.change.length)&&d.change.push(p)}else d.add.push([c,w]);return d}function A(a,b,d,c){var k=a.db.createObjectStore(b,d.keyPath?{keyPath:d.keyPath,autoIncrement:d.auto}:{autoIncrement:d.auto});return c.forEach(function(a){x(k,a)}),k}function wa(a,b){Object.keys(a).forEach(function(d){b.db.objectStoreNames.contains(d)||
A(b,d,a[d].primKey,a[d].indexes)})}function x(a,b){a.createIndex(b.name,b.keyPath,{unique:b.unique,multiEntry:b.multi})}function C(a,b){throw new I("Table "+b[0]+" not part of transaction. Original Scope Function Source: "+g.Promise.PSD.trans.scopeFunc.toString());}function ga(a,b,d,e){this.name=a;this.schema=d;this.hook=W[a]?W[a].hook:v(null,{creating:[f,y],reading:[R,u],updating:[h,y],deleting:[c,y]});this._tpf=b;this._collClass=e||da}function ya(a,b,d,c){ga.call(this,a,b,d,c||J)}function H(a,b,
d,c){function k(a,p,k,b){return w._promise(a,k,b)}var w=this,p,Z;this.db=G;this.mode=a;this.storeNames=b;this.idbtrans=null;this.on=v(this,["complete","error"],"abort");this._reculock=0;this._blockedFuncs=[];this._psd=null;this.active=!0;this._dbschema=d;c&&(this.parent=c);this._tpf=k;this.tables=Object.create(ma);for(c=b.length-1;-1!==c;--c)p=b[c],Z=G._tableFactory(a,d[p],k),this.tables[p]=Z,this[p]||(this[p]=Z)}function na(a,b,d){this._ctx={table:a,index:":id"===b?null:b,collClass:a._collClass,
or:d}}function da(a,b){var d=null,c=null,k;if(b)try{d=b()}catch(w){c=w}k=a._ctx;this._ctx={table:k.table,index:k.index,isPrimKey:!k.index||k.table.schema.primKey.keyPath&&k.index===k.table.schema.primKey.name,range:d,op:"openCursor",dir:"next",unique:"",algorithm:null,filter:null,isMatch:null,offset:0,limit:Infinity,error:c,or:k.or}}function J(){da.apply(this,arguments)}function Q(a,b){return a._cfg.version-b._cfg.version}function Ia(a,b,c,e,k,w){c.forEach(function(p){var c=G._tableFactory(e,k[p],
b);a.forEach(function(a){a[p]||(w?Object.defineProperty(a,p,{configurable:!0,enumerable:!0,get:function(){var a=d.PSD&&d.PSD.trans;return a&&a.db===G?a.tables[p]:c}}):a[p]=c)})})}function Na(a){a.forEach(function(a){for(var b in a)a[b]instanceof ga&&delete a[b]})}function ra(a,b,c,e,k,w){var p=d.PSD;w=w||u;a.onerror||(a.onerror=q(k));a.onsuccess=b?Ka(function(){var p=a.result,d;p?(d=function(){p.continue()},b(p,function(a){d=a},e,k)&&c(w(p.value),p,function(a){d=a}),d()):e()},k,p):Ka(function(){var p=
a.result,k;p?(k=function(){p.continue()},c(w(p.value),p,function(a){k=a}),k()):e()},k,p)}function za(a){var b=[];return a.split(",").forEach(function(a){a=a.trim();var d=a.replace("&","").replace("++","").replace("*",""),k=0!==d.indexOf("[")?d:a.substring(a.indexOf("[")+1,a.indexOf("]")).split("+");b.push(new sa(d,k||null,-1!==a.indexOf("&"),-1!==a.indexOf("*"),-1!==a.indexOf("++"),Array.isArray(k),-1!==k.indexOf(".")))}),b}function aa(a,b){return a<b?-1:a>b?1:0}function Aa(a,b){return a<b?1:a>b?
-1:0}function Ba(a){return function(b,d){for(var c=0,k;;){if(k=a(b[c],d[c]),0!==k)return k;if(++c,c===b.length||c===d.length)return a(b.length,d.length)}}}function ha(a,b){return a?b?function(){return a.apply(this,arguments)&&b.apply(this,arguments)}:a:b}function Ea(){if(G.verno=U.version/10,G._dbSchema=T={},ba=[].slice.call(U.objectStoreNames,0),0!==ba.length){var a=U.transaction(E(ba),"readonly");ba.forEach(function(b){for(var d,c=a.objectStore(b),k=c.keyPath,w=k&&"string"==typeof k&&-1!==k.indexOf("."),
p=new sa(k,k||"",!1,!1,!!c.autoIncrement,k&&"string"!=typeof k,w),e=[],la=0;la<c.indexNames.length;++la)d=c.index(c.indexNames[la]),w=(k=d.keyPath)&&"string"==typeof k&&-1!==k.indexOf("."),d=new sa(d.name,k,!!d.unique,!!d.multiEntry,!1,k&&"string"!=typeof k,w),e.push(d);T[b]=new Ca(b,p,e,{})});Ia([W],G._transPromiseFactory,Object.keys(T),V,T)}}function xa(a,b){for(var d,c,k,w,p=b.db.objectStoreNames,e=0;e<p.length;++e)for(d=p[e],c=b.objectStore(d),k=0;k<c.indexNames.length;++k){var la=c.indexNames[k],
q=c.index(la).keyPath,q="string"==typeof q?q:"["+[].slice.call(q).join("+")+"]";a[d]&&(w=a[d].idxByName[q],w&&(w.name=la))}}var Ha=ja&&ja.addons||g.addons,oa=g.dependencies,pa=oa.indexedDB,Y=oa.IDBKeyRange,ta=oa.TypeError,I=oa.Error,T=this._dbSchema={},ca=[],ba=[],W={},ma={},U=null,ua=!0,qa=null,Da=!1,V="readwrite",G=this,ia=[],va=!0,Ja=!!L();this.version=function(a){if(U)throw new I("Cannot add version when database is open");this.verno=Math.max(this.verno,a);var b=ca.filter(function(b){return b._cfg.version===
...
!1;ia.forEach(function(a){a.resume()});ia=[];b(G)}d.PSD.letThrough=!0;try{var w=G.on.ready.fire();w&&"function"==typeof w.then?w.then(a,function(a){U.close();U=null;e(a)}):D(a)}catch(p){e(p)}})},e)}catch(k){e(k)}})};this.close=function(){U&&(U.close(),U=null,ua=!0,qa=null)};this.delete=function(){var b=arguments;return new d(function(d,c){function e(){G.close();var b=pa.deleteDatabase(a);b.onsuccess=function(){Ja||Oa(function(b){var p=b.indexOf(a);if(0<=p)return b.splice(p,1)});d()};b.onerror=q(c,
["deleting",a]);b.onblocked=function(){G.on("blocked").fire()}}if(0<b.length)throw new I("Arguments not allowed in db.delete()");Da?ia.push({resume:e}):e()})};this.backendDB=function(){return U};this.isOpen=function(){return null!==U};this.hasFailed=function(){return null!==qa};this.dynamicallyOpened=function(){return va};this.name=a;Object.defineProperty(this,"tables",{get:function(){return Object.keys(W).map(function(a){return W[a]})}});this.on=v(this,"error","populate","blocked",{ready:[Ya,y],
versionchange:[X,y]});this.on.ready.subscribe=z(this.on.ready.subscribe,function(a){return function(b,d){function c(){return d||G.on.ready.unsubscribe(c),b.apply(this,arguments)}a.call(this,c);G.isOpen()&&(ua?ia.push({resume:c}):c())}});F(function(){G.on("populate").fire(G._createTransaction(V,ba,T));G.on("error").fire(new I)});this.transaction=function(a,b,c){function e(b,w){var q=null,m,S,f;try{if(p)throw p;q=G._createTransaction(a,Z,T,k);m=Z.map(function(a){return q.tables[a]});m.push(q);f=0;d.newPSD(function(){d.PSD.trans=
q;q.scopeFunc=c;k&&(q.idbtrans=k.idbtrans,q._promise=z(q._promise,function(a){return function(b,p,k){function c(a){return function(b){var p;return d._rootExec(function(){p=a(b);d._tickFinalize(function(){0==--f&&q.active&&(q.active=!1,q.on.complete.fire())})}),p}}return++f,a.call(this,b,function(a,b,d){return p(c(a),c(b),d)},k)}}));q.complete(function(){b(S)});q.error(function(a){q.idbtrans&&(q.idbtrans.onerror=Xa);try{q.abort()}catch(b){}k&&(k.active=!1,k.on.error.fire(a));var p=w(a);k||p||G.on.error.fire(a)});
d._rootExec(function(){S=c.apply(q,m)})});(!q.idbtrans||k&&0===f)&&q._nop()}catch(F){q&&q.idbtrans&&(q.idbtrans.onerror=Xa),q&&q.abort(),k&&k.on.error.fire(F),D(function(){w(F)||G.on("error").fire(F)})}}var k,w;b=[].slice.call(arguments,1,arguments.length-1);c=arguments[arguments.length-1];(k=d.PSD&&d.PSD.trans)&&k.db===G&&-1===a.indexOf("!")||(k=null);w=-1!==a.indexOf("?");a=a.replace("!","").replace("?","");var p=null,Z=(Array.isArray(b[0])?b.reduce(function(a,b){return a.concat(b)}):b).map(function(a){return"string"==
typeof a?a:(a instanceof ga||(p=p||new ta("Invalid type. Arguments following mode must be instances of Table or String")),a.name)});return"r"==a||"readonly"==a?a="readonly":"rw"==a||a==V?a=V:p=new I("Invalid transaction mode: "+a),k&&(p||(k&&"readonly"===k.mode&&a===V&&(w?k=null:p=p||new I("Cannot enter a sub-transaction with READWRITE mode when parent transaction is READONLY")),k&&Z.forEach(function(a){k.tables.hasOwnProperty(a)||(w?k=null:p=p||new I("Table "+a+" not included in parent transaction. Parent Transaction function: "+
k.scopeFunc.toString()))}))),k?k._promise(a,e,"lock"):G._whenReady(e)};this.table=function(a){if(P&&va)return new ya(a);if(!W.hasOwnProperty(a))throw new I("Table does not exist");return W[a]};B(ga.prototype,function(){function a(){throw new I("Current Transaction is READONLY");}return{_trans:function(a,b,d){return this._tpf(a,[this.name],b,d)},_idbstore:function(a,b,c){if(P)return new d(b);var k=this;return this._tpf(a,[this.name],function(a,p,d){b(a,p,d.idbtrans.objectStore(k.name),d)},c)},get:function(a,
b){var d=this;return this._idbstore("readonly",function(b,c,p){P&&b(d.schema.instanceTemplate);var e=p.get(a);e.onerror=q(c,["getting",a,"from",d.name]);e.onsuccess=function(){b(d.hook.reading.fire(e.result))}}).then(b)},where:function(a){return new na(this,a)},count:function(a){return this.toCollection().count(a)},offset:function(a){return this.toCollection().offset(a)},limit:function(a){return this.toCollection().limit(a)},reverse:function(){return this.toCollection().reverse()},filter:function(a){return this.toCollection().and(a)},
each:function(a){var b=this;return P&&a(b.schema.instanceTemplate),this._idbstore("readonly",function(d,c,w){w=w.openCursor();w.onerror=q(c,["calling","Table.each()","on",b.name]);ra(w,null,a,d,c,b.hook.reading.fire)})},toArray:function(a){var b=this;return this._idbstore("readonly",function(a,d,c){P&&a([b.schema.instanceTemplate]);var p=[];c=c.openCursor();c.onerror=q(d,["calling","Table.toArray()","on",b.name]);ra(c,null,function(a){p.push(a)},function(){a(p)},d,b.hook.reading.fire)}).then(a)},
...
c),a},defineClass:function(a){return this.mapToClass(g.defineClass(a),a)},add:a,put:a,"delete":a,clear:a,update:a}});l(ya).from(ga).extend(function(){return{add:function(a,d){var c=this,e=this.hook.creating.fire;return this._idbstore(V,function(k,w,p,Z){var m={},f,S,F;e!==y&&(f=d||(p.keyPath?n(a,p.keyPath):b),S=e.call(m,f,a,Z),f===b&&S!==b&&(p.keyPath?N(a,p.keyPath,S):d=S));F=d?p.add(a,d):p.add(a);F.onerror=q(function(a){if(m.onerror)m.onerror(a);return w(a)},["adding",a,"into",c.name]);F.onsuccess=
function(b){var d=p.keyPath;if(d&&N(a,d,b.target.result),m.onsuccess)m.onsuccess(b.target.result);k(F.result)}})},put:function(a,d){var c=this,e=this.hook.updating.fire;return this.hook.creating.fire!==y||e!==y?this._trans(V,function(k,w,p){var e=d||c.schema.primKey.keyPath&&n(a,c.schema.primKey.keyPath);e===b?p.tables[c.name].add(a).then(k,w):(p._lock(),a=m(a),p.tables[c.name].where(":id").equals(e).modify(function(){this.value=a}).then(function(b){return 0===b?p.tables[c.name].add(a,d):e}).finally(function(){p._unlock()}).then(k,
w))}):this._idbstore(V,function(b,w,p){var e=d?p.put(a,d):p.put(a);e.onerror=q(w,["putting",a,"into",c.name]);e.onsuccess=function(d){var c=p.keyPath;c&&N(a,c,d.target.result);b(e.result)}})},"delete":function(a){return this.hook.deleting.subscribers.length?this.where(":id").equals(a).delete():this._idbstore(V,function(b,d,c){var k=c.delete(a);k.onerror=q(d,["deleting",a,"from",c.name]);k.onsuccess=function(){b(k.result)}})},clear:function(){return this.hook.deleting.subscribers.length?this.toCollection().delete():
this._idbstore(V,function(a,b,d){var c=d.clear();c.onerror=q(b,["clearing",d.name]);c.onsuccess=function(){a(c.result)}})},update:function(a,c){if("object"!=typeof c||Array.isArray(c))throw new I("db.update(keyOrObject, modifications). modifications must be an object.");if("object"!=typeof a||Array.isArray(a))return this.where(":id").equals(a).modify(c);Object.keys(c).forEach(function(b){N(a,b,c[b])});var e=n(a,this.schema.primKey.keyPath);return e===b&&d.reject(new I("Object does not contain its primary key")),
...
a.length&&Array.isArray(a[0])?a[0]:a)}function d(a){return"next"===a?function(a){return a.toUpperCase()}:function(a){return a.toLowerCase()}}function c(a){return"next"===a?function(a){return a.toLowerCase()}:function(a){return a.toUpperCase()}}function k(a,b,d,c,k,e){for(var w,q=Math.min(a.length,c.length),m=-1,f=0;f<q;++f){if(w=b[f],w!==c[f])return 0>k(a[f],d[f])?a.substr(0,f)+d[f]+d.substr(f+1):0>k(a[f],c[f])?a.substr(0,f)+c[f]+d.substr(f+1):0<=m?a.substr(0,m)+b[m]+d.substr(m+1):null;0>k(a[f],w)&&
(m=f)}return q<c.length&&"next"===e?a+d.substr(a.length):q<a.length&&"prev"===e?a.substr(0,d.length):0>m?null:a.substr(0,m)+c[m]+d.substr(m+1)}function e(a,b,w){function q(a){m=d(a);f=c(a);S="next"===a?aa:Aa;F=m(w);n=f(w);ja=a}var m,f,S,F,n,ja;q("next");a._ondirectionchange=function(a){q(a)};a._addAlgorithm(function(a,d,c){var p=a.key,e,w;return"string"!=typeof p?!1:(e=f(p),b(e,n)?(d(function(){a.continue()}),!0):(w=k(p,e,F,n,S,ja),w?d(function(){a.continue(w)}):d(c),!1))})}return{between:function(a,
...
d=a.table.schema,c=(a=a.index?d.idxByName[a.index]:d.primKey)&&a.compound,k=b(arguments),e=c?Ba(aa):aa,w,q;return(k.sort(e),0===k.length)?(new this._ctx.collClass(this,function(){return Y.only("")})).limit(0):(w=new this._ctx.collClass(this,function(){return Y.bound(k[0],k[k.length-1])}),w._ondirectionchange=function(a){e="next"===a?aa:Aa;c&&(e=Ba(e));k.sort(e)},q=0,w._addAlgorithm(function(a,b,d){for(var c=a.key;0<e(c,k[q]);)if(++q,q===k.length)return b(d),!1;return 0===e(c,k[q])?(b(function(){a.continue()}),
!0):(b(function(){a.continue(k[q])}),!1)}),w)},notEqual:function(a){return this.below(a).or(this._ctx.index).above(a)},noneOf:function(){var a=this._ctx,d=a.table.schema,d=(d=a.index?d.idxByName[a.index]:d.primKey)&&d.compound,c=b(arguments),k,e,w,q;return 0===c.length?new this._ctx.collClass(this):(k=d?Ba(aa):aa,c.sort(k),e=c.reduce(function(a,b){return a?a.concat([[a[a.length-1][1],b]]):[[null,b]]},null),e.push([c[c.length-1],null]),w=this,q=a.index,e.reduce(function(a,b){return a?null===b[1]?a.or(q).above(b[0]):
a.or(q).between(b[0],b[1],!1,!1):w.below(b[1])},null))},startsWithAnyOf:function(){function d(a){return a>w[m]}function c(a){return a<e[m]}var k=this._ctx,e=b(arguments),w,q,m,f,S;return e.every(function(a){return"string"==typeof a})?0===e.length?(new k.collClass(this,function(){return Y.only("")})).limit(0):(w=e.map(function(a){return a+String.fromCharCode(65535)}),q=aa,e.sort(q),m=0,f=d,S=new k.collClass(this,function(){return Y.bound(e[0],e[e.length-1]+String.fromCharCode(65535))}),S._ondirectionchange=
function(a){"next"===a?(f=d,q=aa):(f=c,q=Aa);e.sort(q);w.sort(q)},S._addAlgorithm(function(a,b,d){for(var c=a.key;f(c);)if(++m,m===e.length)return b(d),!1;return c>=e[m]&&c<=w[m]?(b(function(){a.continue()}),!0):(b(function(){q===aa?a.continue(e[m]):a.continue(w[m])}),!1)}),S):a(new k.collClass(this),new ta("startsWithAnyOf() only works with strings"))}}});B(da.prototype,function(){function a(b,d){b.filter=ha(b.filter,d)}function b(a,d){a.isMatch=ha(a.isMatch,d)}function c(a,b){if(a.isPrimKey)return b;
...
return P&&a(b.table.schema.instanceTemplate),this._read(function(d,c,e){k(b,a,d,c,e)})},count:function(a){var b,e,m;return P?d.resolve(0).then(a):(b=this,e=this._ctx,e.filter||e.algorithm||e.or?(m=0,this._read(function(a,b,d){k(e,function(){return++m,!1},function(){a(m)},b,d)},a)):this._read(function(a,d,k){k=c(e,k);k=e.range?k.count(e.range):k.count();k.onerror=q(d,["calling","count()","on",b.name]);k.onsuccess=function(b){a(Math.min(b.target.result,Math.max(0,e.limit-e.offset)))}},a))},sortBy:function(a,
b){function d(a,b){return b?d(a[e[b]],b-1):a[k]}function c(a,b){var e=d(a,q),k=d(b,q);return e<k?-m:e>k?m:0}var e=a.split(".").reverse(),k=e[0],q=e.length-1,m="next"===this._ctx.dir?1:-1;return this.toArray(function(a){return a.sort(c)}).then(b)},toArray:function(a){var b=this._ctx;return this._read(function(a,d,c){P&&a([b.table.schema.instanceTemplate]);var e=[];k(b,function(a){e.push(a)},function(){a(e)},d,c)},a)},offset:function(b){var d=this._ctx;return 0>=b?this:(d.offset+=b,d.or||d.algorithm||
d.filter?a(d,function(){return 0>--b}):a(d,function(a,d){return 0===b?!0:1===b?(--b,!1):(d(function(){a.advance(b);b=0}),!1)}),this)},limit:function(b){return this._ctx.limit=Math.min(this._ctx.limit,b),a(this._ctx,function(a,d,c){return 0>=--b&&d(c),0<=b}),this},until:function(b,d){var c=this._ctx;return P&&b(c.table.schema.instanceTemplate),a(this._ctx,function(a,c,e){return b(a.value)?(c(e),d):!0}),this},first:function(a){return this.limit(1).toArray(function(a){return a[0]}).then(a)},last:function(a){return this.reverse().first(a)},
and:function(d){return P&&d(this._ctx.table.schema.instanceTemplate),a(this._ctx,function(a){return d(a.value)}),b(this._ctx,d),this},or:function(a){return new na(this._ctx.table,a,this)},reverse:function(){return this._ctx.dir="prev"===this._ctx.dir?"next":"prev",this._ondirectionchange&&this._ondirectionchange(this._ctx.dir),this},desc:function(){return this.reverse()},eachKey:function(a){var b=this._ctx;return P&&a(n(this._ctx.table.schema.instanceTemplate,this._ctx.index?this._ctx.table.schema.idxByName[this._ctx.index].keyPath:
this._ctx.table.schema.primKey.keyPath)),b.isPrimKey||(b.op="openKeyCursor"),this.each(function(b,d){a(d.key,d)})},eachUniqueKey:function(a){return this._ctx.unique="unique",this.eachKey(a)},keys:function(a){var b=this._ctx,c;return(b.isPrimKey||(b.op="openKeyCursor"),c=[],P)?(new d(this.eachKey.bind(this))).then(function(a){return[a]}).then(a):this.each(function(a,b){c.push(b.key)}).then(function(){return c}).then(a)},uniqueKeys:function(a){return this._ctx.unique="unique",this.keys(a)},firstKey:function(a){return this.limit(1).keys(function(a){return a[0]}).then(a)},
lastKey:function(a){return this.reverse().firstKey(a)},distinct:function(){var b={};return a(this._ctx,function(a){a=a.primaryKey.toString();var d=b.hasOwnProperty(a);return b[a]=!0,!d}),this}}});l(J).from(da).extend({modify:function(a){var b=this,d=this._ctx,c=d.table.hook,e=c.updating.fire,w=c.deleting.fire;return P&&"function"==typeof a&&a.call({value:d.table.schema.instanceTemplate},d.table.schema.instanceTemplate),this._write(function(c,f,S,F){function ja(a){return a&&(P.push(a),A.push(X)),f(new t("Error modifying one or more objects",
P,h,A))}function ea(){v&&h+P.length===D&&(0<P.length?ja():c(h))}var ka,g,r,M;"function"==typeof a?ka=e===y&&w===y?a:function(b){var d=m(b),c,p;if(!1===a.call(this,b))return!1;this.hasOwnProperty("value")?(c=O(d,this.value),p=e.call(this,c,this.primKey,d,F),p&&(b=this.value,Object.keys(p).forEach(function(a){N(b,a,p[a])}))):w.call(this,this.primKey,b,F)}:e===y?(g=Object.keys(a),r=g.length,ka=function(b){for(var d,c,e=!1,k=0;k<r;++k)d=g[k],c=a[d],n(b,d)!==c&&(N(b,d,c),e=!0);return e}):(M=a,a=K(M),ka=
function(b){var d=!1,c=e.call(this,a,this.primKey,m(b),F);return c&&B(a,c),Object.keys(a).forEach(function(c){var e=a[c];n(b,c)!==e&&(N(b,c,e),d=!0)}),c&&(a=K(M)),d});var D=0,h=0,v=!1,P=[],A=[],X=null;b._iterate(function(a,b){var c,e,k;if(X=b.primaryKey,c={primKey:b.primaryKey,value:a},!1!==ka.call(c,a))k=(e=!c.hasOwnProperty("value"))?b.delete():b.update(c.value),++D,k.onerror=q(function(a){if(P.push(a),A.push(c.primKey),c.onerror)c.onerror(a);return ea(),!0},e?["deleting",a,"from",d.table.name]:
["modifying",a,"on",d.table.name]),k.onsuccess=function(){if(c.onsuccess)c.onsuccess(c.value);++h;ea()};else if(c.onsuccess)c.onsuccess(c.value)},function(){v=!0;ea()},ja,S)})},"delete":function(){return this.modify(function(){delete this.value})}});B(this,{Collection:da,Table:ga,Transaction:H,Version:ka,WhereClause:na,WriteableCollection:J,WriteableTable:ya});(function(){G.on("versionchange",function(){G.close();G.on("error").fire(new I("Database version changed by other database connection."))})})();
...
new sa;this.indexes=d||[new sa];this.instanceTemplate=c;this.mappedClass=null;this.idxByName=d.reduce(function(a,b){return a[b.name]=b,a},{})}function t(a,b,d,c){this.name="ModifyError";this.failures=b;this.failedKeys=c;this.successCount=d;this.message=b.join("\n")}function E(a){return 1===a.length?a[0]:a}function L(){var a=g.dependencies.indexedDB,b=a&&(a.getDatabaseNames||a.webkitGetDatabaseNames);return b&&b.bind(a)}var d=function(){function a(b){O.push([b,ea.call(arguments,1)])}function b(){var a=
O,d,c,e;O=[];d=0;for(c=a.length;d<c;++d)e=a[d],e[0].apply(x,e[1])}function d(a){if("object"!=typeof this)throw new TypeError("Promises must be constructed via new");if("function"!=typeof a)throw new TypeError("not a function");this._value=this._state=null;this._deferreds=[];this._catched=!1;var b=this,c=!0;this._PSD=d.PSD;try{n(this,a,function(a){c?r(q,b,a):q(b,a)},function(a){return c?(r(m,b,a),!1):m(b,a)})}finally{c=!1}}function c(q,m){var f,F,n,ea,t,D;if(null===q._state)q._deferreds.push(m);else{if(f=
q._state?m.onFulfilled:m.onRejected,null===f)return(q._state?m.resolve:m.reject)(q._value);n=M;M=!1;r=a;try{ea=d.PSD,d.PSD=q._PSD,F=f(q._value),q._state||F&&"function"==typeof F.then&&!1===F._state||e(q),m.resolve(F)}catch(v){if(t=m.reject(v),!t&&q.onuncatched)try{q.onuncatched(v)}catch(P){}}finally{if(d.PSD=ea,n){do{for(;0<O.length;)b();if(D=h.pop(),D)try{D()}catch(v){}}while(0<h.length||0<O.length);r=g;M=!0}}}}function e(a){a._catched=!0;a._parent&&e(a._parent)}function q(a,b){var c=d.PSD;d.PSD=
...
{};try{return a()}finally{d.PSD=b}},d._rootExec=function(d){var c=M,e;M=!1;r=a;try{d()}finally{if(c){do{for(;0<O.length;)b();if(e=h.pop(),e)try{e()}catch(q){}}while(0<h.length||0<O.length);r=g;M=!0}}},d._tickFinalize=function(a){if(M)throw Error("Not in a virtual tick");h.push(a)},d}(),F=function(){},P=!1,wa;l(t).from(Error);g.delete=function(a){var b=new g(a);a=b.delete();return a.onblocked=function(a){b.on("blocked",a);return this},a};g.exists=function(a){return(new g(a)).open().then(function(a){return a.close(),
!0},function(){return!1})};g.getDatabaseNames=function(a){return(new d(function(a,b){var d=L(),c;d?(c=d(),c.onsuccess=function(b){a([].slice.call(b.target.result,0))},c.onerror=q(b)):Oa(function(b){return a(b),!1})})).then(a)};g.defineClass=function(a){return function(b){b?B(this,b):P&&e(this,a)}};g.ignoreTransaction=function(a){return d.newPSD(function(){return d.PSD.trans=null,a()})};g.spawn=function(){return x.console&&console.warn("Dexie.spawn() is deprecated. Use Dexie.ignoreTransaction() instead."),
g.ignoreTransaction.apply(this,arguments)};g.vip=function(a){return d.newPSD(function(){return d.PSD.letThrough=!0,a()})};Object.defineProperty(g,"currentTransaction",{get:function(){return d.PSD&&d.PSD.trans||null}});g.Promise=d;g.derive=l;g.extend=B;g.override=z;g.events=v;g.getByKeyPath=n;g.setByKeyPath=N;g.delByKeyPath=function(a,d){"string"==typeof d?N(a,d,b):"length"in d&&[].map.call(d,function(d){N(a,d,b)})};g.shallowClone=K;g.deepClone=m;g.addons=[];g.fakeAutoComplete=F;g.asap=D;g.ModifyError=
...
typeof chrome&&null!==chrome?chrome.storage:void 0)?null:x.localStorage};g.version=1.2;C("Dexie",g);A(function(){g.fakeAutoComplete=F=A;g.fake=P=!0})}).apply(null,[self||window,function(x,C){!(L=function(){return C}.call(E,J,E,Q),void 0!==L&&(Q.exports=L))}])}).call(E,J(2).setImmediate)},function(Q,E,J){(function(L,H){function x(b,l){this._id=b;this._clearFn=l}var C=J(3).nextTick,b=Function.prototype.apply,B=Array.prototype.slice,l={},z=0;E.setTimeout=function(){return new x(b.call(setTimeout,window,
...
g(arguments);return b.setTimeout(l(z,a),0),a}};var v=Object.getPrototypeOf&&Object.getPrototypeOf(b),v=v&&v.setTimeout?v:b;R?v.setImmediate=r.setTimeout():"[object process]"===h(b.process)?v.setImmediate=r.nextTick():u()?v.setImmediate=r.postMessage():b.MessageChannel?v.setImmediate=r.messageChannel():a&&"onreadystatechange"in a.createElement("script")?v.setImmediate=r.readyStateChange():v.setImmediate=r.setTimeout();v.msSetImmediate=v.setImmediate;v.clearImmediate=y;v.msClearImmediate=y})(function(){return this||
(0,eval)("this")}());(function(b,B){function l(a){Object.defineProperties(this,{_subscribers:{value:[]},resolve:{value:this.resolve.bind(this)},reject:{value:this.reject.bind(this)}});var b=this;"function"==typeof a&&N(function(){try{b.resolve(a())}catch(c){b.reject(c)}})}function z(a){return a instanceof l}function g(a){return z(a)||n&&a instanceof b.Promise&&typeof("function"===a.then)}function y(a){var b=new l;return b.reject(a),b}function u(a){var b=new l;return b.resolve(a),b}function R(a){var b=
0;return O(a,function(){++b}),b}function a(a){return a=g(a)?a:u(a),a.then(function(a){var b=a.length,c=R(a),e=Array(b);if(0===c)return u(e);var f=new l;return O(a,function(a,b){(g(a)?a:u(a)).then(function(a){e[b]=a;0===--c&&f.resolve(e)},f.reject)}),f})}function f(a){return a=g(a)?a:u(a),a.then(function(a){var b=new l;return O(a,function(a){(g(a)?a:u(a)).then(b.resolve,b.reject)}),b})}function h(b,c,f){return a(b).then(function(a){return a.forEach(c,f)})}function r(b,c,f){return a(b).then(function(a){return a.every(c,
f)})}function X(b,c,f){return a(b).then(function(a){return a.some(c,f)})}function c(b,c,f){return a(b).then(function(a){return a.filter(c,f)})}function x(b,c,f){return a(b).then(function(a){return a.map(c,f)})}function v(b,c,f){return 2<arguments.length?a(b).then(function(a){return g(f)||(f=u(f)),f.then(function(b){return a.reduce(c,b)})}):a(b).then(function(a){return a.reduce(c)})}function D(b,c,f){return 2<arguments.length?a(b).then(function(a){return g(f)||(f=u(f)),f.then(function(b){return a.reduceRight(c,
b)})}):a(b).then(function(a){return a.reduceRight(c)})}function A(a,b,c){N(function(){try{var f=a(c);b.resolve(f)}catch(m){b.reject(m)}})}function C(a,b,c,f,m){function n(m){C(a,b,c,f,m)}function h(a){b?A(b,f,a):f.reject(a)}if(g(m))return m===c?void h(new TypeError("Self resolution")):void m.then(n,h);if(null!==m&&"object"==typeof m||"function"==typeof m){var r;try{r=m.then}catch(M){return void h(M)}if("function"==typeof r){var O=!0;try{return void r.call(m,function(a){O&&(O=!1,n(a))},function(a){O&&
(O=!1,h(a))})}catch(M){O&&(O=!1,h(M))}return}}a?A(a,f,m):f.resolve(m)}var n="Promise"in b,N=b.setImmediate,K=b.setTimeout,m=b.clearTimeout,O=Function.prototype.call.bind(Array.prototype.forEach),M=Function.prototype.call.bind(Array.prototype.slice);Object.defineProperties(l,{delayed:{value:function(a,b){var c="function"==typeof b?b:function(){return b},f=new l;return K(function(){try{f.resolve(c())}catch(a){f.reject(a)}},a),f}},error:{value:y},sync:{value:function(a){try{var b=a();return u(b)}catch(c){return y(c)}}},
value:{value:u},all:{value:a},race:{value:f},resolve:{value:u},reject:{value:y},promise:{value:function(a){var b=new l;return a(b.resolve,b.reject),b}},isFuture:{value:z},isPromise:{value:g},join:{value:function(){return a(arguments)}},any:{value:function(a){return a=g(a)?a:u(a),a.then(function(a){var b=a.length,c=R(a);if(0===c)throw new RangeError("any(): array must not be empty");var e=Array(b),f=new l;return O(a,function(a,b){(g(a)?a:u(a)).then(f.resolve,function(a){e[b]=a;0===--c&&f.reject(e)})}),
f})}},settle:{value:function(a){return a=g(a)?a:u(a),a.then(function(a){var b=a.length,c=R(a),e=Array(b);if(0===c)return u(e);var f=new l;return O(a,function(a,b){var m=g(a)?a:u(a);m.whenComplete(function(){e[b]=m.inspect();0===--c&&f.resolve(e)})}),f})}},attempt:{value:function(b){var c=M(arguments,1);return a(c).then(function(a){return b.apply(B,a)})}},run:{value:function(b,c){var f=M(arguments,2);return a(f).then(function(a){return b.apply(c,a)})}},wrap:{value:function(b,c){return function(){return a(arguments).then(function(a){return b.apply(c,
a)})}}},forEach:{value:h},every:{value:r},some:{value:X},filter:{value:c},map:{value:x},reduce:{value:v},reduceRight:{value:D}});Object.defineProperties(l.prototype,{_value:{writable:!0},_reason:{writable:!0},_state:{value:0,writable:!0},resolve:{value:function(a){if(0===this._state){this._state=1;this._value=a;for(var b=this._subscribers;0<b.length;){var c=b.shift();C(c.onfulfill,c.onreject,this,c.next,a)}}}},reject:{value:function(a){if(0===this._state){this._state=2;this._reason=a;for(var b=this._subscribers;0<
...
g&&l>c;g++){var v=a[c++];switch(v>>4){case 0:case 1:case 2:case 3:case 4:case 5:case 6:case 7:h[g]=v;break;case 12:case 13:if(!(l>c))throw Error("Unfinished UTF-8 octet sequence");h[g]=(31&v)<<6|63&a[c++];break;case 14:if(!(l>c+1))throw Error("Unfinished UTF-8 octet sequence");h[g]=(15&v)<<12|(63&a[c++])<<6|63&a[c++];break;case 15:if(!(l>c+2))throw Error("Unfinished UTF-8 octet sequence");v=((7&v)<<18|(63&a[c++])<<12|(63&a[c++])<<6|63&a[c++])-65536;if(!(0<=v&&1048575>=v))throw Error("Character outside valid Unicode range: 0x"+
v.toString(16));h[g++]=v>>10&1023|55296;h[g]=1023&v|56320;break;default:throw Error("Bad UTF-8 encoding 0x"+v.toString(16));}}r=(r>g&&(h=h.subarray(0,g)),[String.fromCharCode.apply(String,h),c])}else{h=b;r=[];g=new Uint16Array(65535);c=v=0;for(l=a.length;h>v&&l>c;v++){var u=a[c++];switch(u>>4){case 0:case 1:case 2:case 3:case 4:case 5:case 6:case 7:g[v]=u;break;case 12:case 13:if(!(l>c))throw Error("Unfinished UTF-8 octet sequence");g[v]=(31&u)<<6|63&a[c++];break;case 14:if(!(l>c+1))throw Error("Unfinished UTF-8 octet sequence");
g[v]=(15&u)<<12|(63&a[c++])<<6|63&a[c++];break;case 15:if(!(l>c+2))throw Error("Unfinished UTF-8 octet sequence");u=((7&u)<<18|(63&a[c++])<<12|(63&a[c++])<<6|63&a[c++])-65536;if(!(0<=u&&1048575>=u))throw Error("Character outside valid Unicode range: 0x"+u.toString(16));g[v++]=u>>10&1023|55296;g[v]=1023&u|56320;break;default:throw Error("Bad UTF-8 encoding 0x"+u.toString(16));}65534<=v&&(v+=1,r.push(String.fromCharCode.apply(String,g.subarray(0,v))),h-=v,v=-1)}r=(0<v&&r.push(String.fromCharCode.apply(String,
...
1)).slice(-2),g=("00"+b.getDate()).slice(-2),h=("00"+b.getHours()).slice(-2),l=("00"+b.getMinutes()).slice(-2),r=("00"+b.getSeconds()).slice(-2),u=("000"+b.getMilliseconds()).slice(-3);"00"===h&&"00"===l&&"00"===r&&"000"===u?(c.writeByte(n.TagDate),c.writeAsciiString(e+f+g)):"1970"===e&&"01"===f&&"01"===g?(c.writeByte(n.TagTime),c.writeAsciiString(h+l+r),"000"!==u&&(c.writeByte(n.TagPoint),c.writeAsciiString(u))):(c.writeByte(n.TagDate),c.writeAsciiString(e+f+g),c.writeByte(n.TagTime),c.writeAsciiString(h+
l+r),"000"!==u&&(c.writeByte(n.TagPoint),c.writeAsciiString(u)));c.writeByte(n.TagSemicolon)}function h(a,b){a._refer.set(b);var c=a.stream,e=("00"+b.getHours()).slice(-2),f=("00"+b.getMinutes()).slice(-2),g=("00"+b.getSeconds()).slice(-2),h=("000"+b.getMilliseconds()).slice(-3);c.writeByte(n.TagTime);c.writeAsciiString(e+f+g);"000"!==h&&(c.writeByte(n.TagPoint),c.writeAsciiString(h));c.writeByte(n.TagSemicolon)}function r(a,b){a._refer.set(b);var c=a.stream;c.writeByte(n.TagBytes);var e=b.byteLength||
...
var c=a.stream,e=[],f;for(f in b)b.hasOwnProperty(f)&&"function"!=typeof b[f]&&(e[e.length]=f);f=e.length;c.writeByte(n.TagMap);0<f&&c.writeAsciiString(""+f);c.writeByte(n.TagOpenbrace);for(var g=0;f>g;g++)y(a,e[g]),y(a,b[e[g]]);c.writeByte(n.TagClosebrace)}function v(a,b){a._refer.set(b);var c=a.stream,e=b.size;c.writeByte(n.TagMap);0<e&&c.writeAsciiString(""+e);c.writeByte(n.TagOpenbrace);b.forEach(function(b,c){y(a,c);y(a,b)});c.writeByte(n.TagClosebrace)}function D(a,b){var c,e,f=a.stream;e=l(b);
if(e in a._classref)e=a._classref[e],c=a._fieldsref[e];else{c=[];for(var g in b)b.hasOwnProperty(g)&&"function"!=typeof b[g]&&(c[c.length]=g.toString());g=c;var h=a.stream,u=g.length;h.writeByte(n.TagClass);h.writeAsciiString(""+e.length);h.writeByte(n.TagQuote);h.writeString(e);h.writeByte(n.TagQuote);0<u&&h.writeAsciiString(""+u);h.writeByte(n.TagOpenbrace);for(var r=0;u>r;r++)x(a,g[r]);h.writeByte(n.TagClosebrace);h=a._fieldsref.length;e=(a._classref[e]=h,a._fieldsref[h]=g,h)}f.writeByte(n.TagObject);
f.writeAsciiString(""+e);f.writeByte(n.TagOpenbrace);a._refer.set(b);e=c.length;for(g=0;e>g;g++)y(a,b[c[g]]);f.writeByte(n.TagClosebrace)}var A=b.Map,E=b.hprose.BytesIO,n=b.hprose.Tags,N=b.hprose.ClassManager,K=Object.create(null,{set:{value:function(){}},write:{value:function(){return!1}},reset:{value:function(){}}});Object.defineProperties(z.prototype,{_refcount:{value:0,writable:!0},set:{value:function(a){this._ref.set(a,this._refcount++)}},write:{value:function(a){a=this._ref.get(a);return a!==
...
writeListWithRef:{value:function(a){this._refer.write(a)||c(this,a,y)}},writeMap:{value:function(a){a instanceof A?v(this,a):C(this,a)}},writeMapWithRef:{value:function(a){this._refer.write(a)||(a instanceof A?v(this,a):C(this,a))}},writeObject:{value:function(a){D(this,a)}},writeObjectWithRef:{value:function(a){this._refer.write(a)||D(this,a)}},reset:{value:function(){this._classref=Object.create(null);this._fieldsref.length=0;this._refer.reset()}}});b.hprose.Writer=g})(function(){return this||(0,eval)("this")}());
...
H(this);break a;default:l(a)}a=void 0}return a}},readTimeWithoutTag:{value:function(){return v(this)}},readTime:{value:function(){var a;a:{a=this.stream.readByte();switch(a){case t.TagNull:a=null;break a;case t.TagTime:a=v(this);break a;case t.TagRef:a=H(this);break a;default:l(a)}a=void 0}return a}},readBytesWithoutTag:{value:function(){return D(this)}},readBytes:{value:function(){return A(this)}},readStringWithoutTag:{value:function(){return n(this)}},readString:{value:function(){return N(this)}},
...
readMapWithoutTag:{value:function(){return this.useHarmonyMap?M(this):O(this)}},readMap:{value:function(){var a;if(this.useHarmonyMap)a:{a=this.stream.readByte();switch(a){case t.TagNull:a=null;break a;case t.TagMap:a=M(this);break a;case t.TagRef:a=H(this);break a;default:l(a)}a=void 0}else a:{a=this.stream.readByte();switch(a){case t.TagNull:a=null;break a;case t.TagMap:a=O(this);break a;case t.TagRef:a=H(this);break a;default:l(a)}a=void 0}return a}},readObjectWithoutTag:{value:function(){return e(this)}},
...
l})(function(){return this||(0,eval)("this")}());(function(b){b.hprose.ResultMode={Normal:0,Serialized:1,Raw:2,RawWithEndTag:3};b.hprose.Normal=b.hprose.ResultMode.Normal;b.hprose.Serialized=b.hprose.ResultMode.Serialized;b.hprose.Raw=b.hprose.ResultMode.Raw;b.hprose.RawWithEndTag=b.hprose.ResultMode.RawWithEndTag})(function(){return this||(0,eval)("this")}());(function(b,B){function l(){}function z(g,z,K){function m(a,b){for(var c=0,d=I.length;d>c;c++)a=I[c].outputFilter(a,b);return a}function O(a,
...
B&&(b[d]={},(typeof e===v||e.constructor===Object)&&(e=[e]),Array.isArray(e)))for(var f=0;f<e.length;f++){var g=e[f];if(typeof g===v)b[d][g]=J(a,c+d+"_"+g);else for(var h in g)Q(a,b[d],d+"_",h,g[h])}}function sa(a,b){for(var c=0;c<b.length;c++){var d=b[c];if(typeof d===v)a[d]===B&&(a[d]=J(a,d));else for(var e in d)Q(a,a,"",e,d[e])}}function Ca(a,b){for(var c=Math.min(a.length,b.length),d=0;c>d;++d)b[d]=a[d]}function t(b,c,d){var e=new a;e.writeByte(u.TagCall);var g=new f(e,d.simple);return g.writeString(b),
...
m=n.readByte());if(0>[u.TagEnd,u.TagResult,u.TagError].indexOf(m))return void g(Error("Wrong Response:\r\n"+a.toString(d)));r>=x.Raw?(r===x.RawWithEndTag&&l.writeByte(u.TagEnd),b[e].result=l.bytes):b[e].result=l;b[e].error=q}}catch(v){return void g(v)}f(b)},g)})}function Ua(a){if(a=a||{},T=!1,oa)return r.promise(function(b,c){pa.push({batch:!0,settings:a,resolve:b,reject:c})});if(0!==ca.length){var b=ka(a);b.sync&&(oa=!0);var c=ca;ca=[];var e=r.promise(function(a,e){qa(c,b).then(function(b){b.forEach(function(a){if(a.error)d(a.name,
...
f){var g=new XMLHttpRequest;g.open("POST",D.uri,!0);b.location!==x&&"file:"!==b.location.protocol&&(g.withCredentials="true");g.responseType="arraybuffer";for(var h in c)g.setRequestHeader(h,c[h]);return g.onload=function(){g.onload=l;g.status&&(200===g.status?f.resolve(new Uint8Array(g.response)):f.reject(Error(g.status+":"+g.statusText)))},g.onerror=function(){f.reject(Error("error"))},g.upload!==x&&(g.upload.onprogress=E),g.onprogress=v,a.constructor===String||ArrayBuffer.isView?g.send(a):a.buffer.slice?
g.send(a.buffer.slice(0,a.length)):g.send(a.buffer),g}function r(a){"function"==typeof a&&(E=a)}function C(){return E}if(this.constructor!==z)return new z(g,a,f);y.call(this,g,a,f);var c=Object.create(null),E=l,v=l,D=this;Object.defineProperties(this,{onProgress:{get:C,set:r},onprogress:{get:C,set:r},onRequestProgress:{get:C,set:r},onResponseProgress:{get:function(){return v},set:function(a){"function"==typeof a&&(v=a)}},setHeader:{value:function(a,b){"content-type"!==a.toLowerCase()&&"content-length"!==
...
function E(a){N.resolve(a)}function c(a){a=new u(a.data);var b=a.readInt32BE(),c=J[b];if(delete J[b],c!==x&&(--A,c.resolve(a.read(a.length-4))),100>A&&0<n.length){++A;var f=n.shift();N.then(function(){r(f[0],f[1])})}0===A&&(m.keepAlive||v())}function H(a){J.forEach(function(b,c){b.reject(Error(a.code+":"+a.reason));delete J[c]});A=0;K=null}function v(){null!==K&&(K.onopen=l,K.onmessage=l,K.onclose=l,K.close())}if(this.constructor!==z)return new z(a,b,g);y.call(this,a,b,g);var D=0,A=0,J=[],n=[],N=
null,K=null,m=this;Object.defineProperties(this,{sendAndReceive:{value:function(a,b){if(null===K||K.readyState===WebSocket.CLOSING||K.readyState===WebSocket.CLOSED)N=new C,K=new WebSocket(m.uri),K.binaryType="arraybuffer",K.onopen=E,K.onmessage=c,K.onerror=l,K.onclose=H;var e=2147483647>D?++D:D=0,f=new C;return J[e]=f,0<m.timeout&&(f=f.timeout(m.timeout).catchError(function(a){throw delete J[e],--A,a;},function(a){return a instanceof L})),100>A?(++A,N.then(function(){r(e,a)})):n.push([e,a]),b.oneway&&
f.resolve(),f}},close:{value:v}})}function g(a){var b=document.createElement("a");if(b.href=a,"ws:"!==b.protocol&&"wss:"!==b.protocol)throw Error("This client desn't support "+b.protocol+" scheme.");}var y=b.hprose.Client,u=b.hprose.BytesIO,C=b.hprose.Future;Object.defineProperty(z,"create",{value:function(a,b,h){if("string"==typeof a)g(a);else{if(!Array.isArray(a))return Error("You should set server uri first!");a.forEach(function(a){g(a)})}return new z(a,b,h)}});b.hprose.WebSocketClient=z})(function(){return this||
