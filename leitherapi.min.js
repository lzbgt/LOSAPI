...
debug.log("getdbdata2 ",a.target.result)};return D}function g(a){var b=c.LeitherDb.transaction("res","readwrite").objectStore("res"),D=new hprose.Future;request=b.put(a);request.onerror=function(a){debug.log("setdbdata err",a);D.reject(a)};request.onsuccess=function(a){D.resolve(a.target.result);debug.log("setdbdata: ",a.target.result)};return D}function y(c,b,D){var f=new hprose.Future,g=function(a,c){var b="last";if("last"==c||"release"==c)b=a[c];return a.ResFile[b]}(JSON.parse(D),b);(function(b){for(var D=
[];b<g.length;b++){var w=g[b];""==w?debug.error("invliad key:",w):D.push(a(c,w))}hprose.Future.all(D).then(function(a){debug.log("all promise",a);f.resolve()},function(a){debug.log(a);f.reject(a)})})(0);return f}function u(a){c.loginData=a;c.sid=a.sid;c.user=a.user;c.username=c.user.name;c.bid=null!==a.user?a.user.id:c.uid;c.leClient=c.api;c.swarm=a.swarm}function R(a){0<a.length?c.currentIP=a:a=c.currentIP;c.api=r(a);l().then(function(a){debug.log("hprose ready",a);c.api.ready(function(a){debug.log("hprose ready ok");
debug.log("G.uid=",c.uid);f(a,c.AppName,c.AppBid,c.AppVer).then(function(){var w=x("login");c.postLogin=u;"undefined"!==typeof ENABLE_UAC&&ENABLE_UAC?b("UAC enabled, must redirect to the login page first"):a.login(c.uid,c.userppt).then(function(a){debug.log("login ok");u(a);c.swarm=a.swarm;debug.log("LeitherIsOK:",C());debug.log("login ok sid=",a.sid);debug.log("user= ",a.user);debug.log("swarm=",a.swarm);debug.log("appName=",c.AppName);b("after loaded app and logedin")},w);c.Local||(debug.log("use remote files"),
a.getresbyname(c.sid,c.SystemBid,"LeitherApi",c.AppVer,{handler:icache.handler,ttl:0}).then(function(a){var b=new FileReader;b.onload=function(a){debug.log("leitherApi re get ok");localStorage.leitherApi=a.target.result};b.readAsText(new Blob([a]))}),a.hget(c.sid,c.AppBid,"applist",c.AppName,{handler:icache.handler,ttl:0}).then(function(a){debug.log("manifest re hget ok");g({id:c.AppName,data:a,tbname:c.ApptbName}).then()}))})},x("api.ready"))},x("InitDb"))}function a(a,b){var f=new hprose.Future,
h=document.createElement("script");h.type="text/javascript";z(b).then(function(r){if(r)debug.log("load js from db: ",b),h.textContent=r.data,document.getElementsByTagName("head")[0].appendChild(h),f.resolve(b);else if(debug.log("check leither"),"function"==typeof C&&C()){debug.log("check leither ok");var n=function(a){debug.error(a);f.reject(b)};c.api.ready(function(r){r.get("",a,b,function(n){debug.log("load js from server: ",a,b);if(n){var m=new FileReader;m.onload=function(a){g({id:b,data:a.target.result,
tbname:c.ApptbName});h.textContent=a.target.result;document.getElementsByTagName("head")[0].appendChild(h);f.resolve(b)};m.readAsText(new Blob([n]))}else debug.error("data is null"),f.reject(b)},n)},n)}else debug.error("leither is not ok"),f.reject(b)},function(a){debug.error(a);f.reject(a)});return f}function f(a,b,f,h){debug.log("load app name:",b,", appBid:",f);var r=new hprose.Future;if(c.Local)return debug.log("use local file"),r.resolve(),r;z(b).then(function(c){c?(debug.log("get app manifest from db successed: ",
b),y(f,h,c.data).then(function(){r.resolve()},function(a){debug.warn("LoadApp: ",a);r.reject(a)})):(debug.log("app :",b," resfiles not found in DB, fetching ... "),a.hget("",f,"applist",b).then(function(a){g({id:b,data:a,tbname:f+"_"+b});y(f,h,a).then(function(a){r.resolve()},function(a){debug.warn("LoadApp 2: ",a);r.reject(a)})}))},function(a){debug.error(a)});return r}var h=J(1);J(4);"use strict";var r=function(a){if("http://"==a.substr(0,7)){a=a.substr(7);var b=a.indexOf("/");0<b&&(a=a.substr(0,
